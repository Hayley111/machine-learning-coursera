...
yVec = yVec(:);
hThetaX = hThetaX(:);
J = -1/m * (yVec' *log(hThetaX)+(1-yVec)'* log(1-hThetaX));

t1 = [zeros(size(Theta1,1),1) Theta1(:,2:end)];
t2 = [zeros(size(Theta2,1),1) Theta2(:,2:end)];

regularator = (sum(sum(t1.^2))+sum(sum(t2.^2)))* (lambda/(2*m));
...
for t = 1:m
	...
	a1 = [1; X(t,:)']; % X already have a bias Line 44 (1*401);% (401*1)

	% For the hidden layers, where l=2:
	z2 = Theta1 * a1;% (25*401)*(401*1);% (25*1)
	a2 = [1; sigmoid(z2)];% adding a bias (26*1)
